...
void *thread_Speaker(void *arg)
{
	...
	int resend = 0;
	//int sendCnt = 0;
	FILE *fp = fopen("audio_raw/beethoven_8k_16bit_mono.raw", "rb");
	if(fp == NULL)
	{
		printf("fopen error!!!\n");
		return 0;
	...
	int avIndex = avServStart3(ServerInfo->SID, AuthCallBackFn, 50, 0, ServerInfo->speakerCh, &resend);
	if(avIndex < 0)
	{
	...
	frameInfo.flags = (AUDIO_SAMPLE_8K << 2) | (AUDIO_DATABITS_16 << 1) | AUDIO_CHANNEL_MONO;
		...
		while(1)
		{
			...
			int ret = avSendAudioData(avIndex, buf, 640, &frameInfo, sizeof(FRAMEINFO_t));
...
#if ENABLE_WAKEUP
    int retry = 0;
    if(SID == IOTC_ER_DEVICE_IS_SLEEP){
        IOTC_Session_Close(tmpSID);

        ret = IOTC_WakeUp_WakeDevice(ServerInfo->UID);
        if(ret < 0){
            printf("IOTC_WakeUp_WakeDevice failed[%d]\n", ret);
            return 0;
        }
        ...
        while(1){
            ...
            printf("  [] thread_ConnectCCR::IOTC_Connect_ByUID_Parallel, ret=[%d]\n", SID);
            ServerInfo->SID = SID;
            if(SID == IOTC_ER_DEVICE_IS_SLEEP && retry < 20){
                IOTC_Session_Close(tmpSID);
                sleep(1);
                retry++;
                continue;
