...
void *thread_ReceiveAudio(void *arg)
{
	int SID = *((int *)arg);
	free(arg);
	int recordCnt = 0, recordFlag = 0, nResend = 0;
	AV_Client *p = &gClientInfo[SID];
...
void Handle_IOCTRL_Cmd(int SID, int avIndex, char *buf, int type)
{
	...
	switch(type)
	{
		case IOTYPE_USER_IPCAM_START:
		{
            ...
            //For DASA -- start
            #ifdef DASA_ENABLED
            avDASAReset(avIndex, AV_DASA_LEVEL_QUALITY_LOW);
            #endif
            //For DASA -- end
		}
		break;
...
void *thread_ForAVServerStart(void *arg)
{
    ...
    //For DASA -- start
    #ifdef DASA_ENABLED
    avServSetResendSize(avIndex, 1024*4);
    int nCleanBufferCondition = 5, nCleanBufferRatio = 70, nAdjustmentKeepTime = 5, nIncreaseQualityCond = 10, nDecreaseRatio = 50;
    ret = avDASASetting(avIndex, 1, nCleanBufferCondition, nCleanBufferRatio, nAdjustmentKeepTime, nIncreaseQualityCond, nDecreaseRatio, AV_DASA_LEVEL_QUALITY_LOW);
    if(ret < 0){
        printf("avDASASetting Error avIndex[%d], Resend[%d], ret[%d]\n\n", avIndex, nResend, ret);
    }
    #endif
    //For DASA -- end
...
void *thread_DASACheck(void *arg)
{
    ...
    while(gProcessRun)
    {
        ...
        if((gOnlineNum == 0 || clientEnableVideoCount == 0) && gDASALevel != AV_DASA_LEVEL_QUALITY_LOW){
            ...
            Emulater_ChangeStream(gDASALevel);
            #endif
...
int Streamout_SendVideoFunc(int streamID, int timestamp, char* buf, int size, int frameType)
{
    ...
    int i = 0, avIndex = 0, bEnableVideo = 0, ret = 0, lock_ret = 0;
    FRAMEINFO_t frameInfo;
    //For DASA -- start
#ifdef DASA_ENABLED
    int doCleanBuffer = 0, doCleanBufferDone = 0, waitKeyFrame = 0, bSendCache = 0;
#endif
    //For DASA -- start
    char *videoBuf = buf;
    int videoSize = size;
    ...
    for(i = 0 ; i < MAX_CLIENT_NUMBER; i++)
    {
        ...
        doCleanBufferDone = gClientInfo[i].doCleanBufferDone;
        waitKeyFrame = gClientInfo[i].waitKeyFrame;
        bSendCache = gClientInfo[i].bSendCache;
        #endif
        //For DASA -- end
		...
		// Send Video Frame to av-idx and know how many time it takes
		frameInfo.onlineNum = gOnlineNum;
		ret = avSendFrameData(avIndex, videoBuf, videoSize, &frameInfo, sizeof(FRAMEINFO_t));
...
#ifdef ENABLE_EMULATER
    ret = Emulater_Initialize(EMULATER_MODE_SINGLESTREAM, AV_DASA_LEVEL_QUALITY_LOW, Streamout_SendVideoFunc, Streamout_SendAudioFunc);
    if(ret < 0){
        printf("Emulater_Initialize ret=%d\n", ret);
        exit(-1);
    }
#else
...
int main(int argc, char *argv[])
{
    ...
    if(argc < 2)
	{
		...
		printf("Usage: ./AVAPIs_Server UID\n");
		return -1;
	}
