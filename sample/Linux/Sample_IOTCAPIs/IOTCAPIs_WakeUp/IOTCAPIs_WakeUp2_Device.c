...
IOTC_WakeUpData *gAliveData = NULL;
unsigned int gDataCnt = 0;
...
{
    return gAliveData ? 1:0;
}

void SleepPacketCB(IOTC_WakeUpData *pData, unsigned int DataCnt, int nErrCode)
{
    printf("[%s] err [%d], cnt [%d]\n", __func__, nErrCode, DataCnt);
    gDataCnt = DataCnt;
    gAliveData = pData;
}
...
int main(int argc, char *argv[])
{
    ...
    //IOTC_WakeUp_Get_SleepPacket "Must" be used after login
    nRet = IOTC_WakeUp_Get_SleepPacket((IOTC_WakeUp_SleepPacketCB)SleepPacketCB, 0);
    if (nRet != IOTC_ER_NoERROR)
    {
        printf("IOTC_WakeUp_Get_SleepPacket error[%d]\n", nRet);
        return 0;
    }
    ...
    pIOTCAliveData = (IOTC_WakeUpData*)malloc(sizeof(IOTC_WakeUpData) * gDataCnt);
    if(pIOTCAliveData)
        duplicateWakeUpData(pIOTCAliveData, gAliveData, gDataCnt);
    else
        printf("malloc error!\n");

    // Free gAliveData memory
    IOTC_WakeUp_DeInit(gAliveData);
    // DeInit IOTC
    IOTC_DeInitialize();
    // ------ Devicde at normal mode ------
    ...
    WakeupParam *pParam = (WakeupParam*)malloc(sizeof(WakeupParam) * gDataCnt);

    for(int i = 0; i < gDataCnt; i++)
    {
        pthread_t nTaskId;
    ...
    free(pParam);
    releaseWakeUpData(pIOTCAliveData, gDataCnt);
    // ------ Device enter sleep mode ------
